...
/*The program has a time complexity of O(n + m + k),
where n is the number of rows, m is the number of operations,
and k is the total number of items in all columns. Most operations
like adding, deleting, or searching go through the rows or columns once,
so they take linear time. There are no heavy nested loops,
so the program runs efficiently even with a fair amount of data.*/
...
// procedure to search the previous row element
...
// procedure to move to first place on another row
void moveToFirstCol_OnAnotherRow(eRow* old, eColumn* x, eRow* new){
    eColumn* prevX; // declare prevX to save element before x
    if(x != old->col){ // if x its not the first element of the row
        searchPrevCol(&prevX, &x, old); // call procedure searchPrevCol to get the previous element
        prevX->next_col = x->next_col; // prevX point to element after x
    }else{ // if x its the first element of the row
        old->col = x->next_col; // update the first element with element after x
    }
    x->next_col = new->col; // x point to the first element of another another row
    new->col = x; // update the first element with x
}

// procedure to move columt element to after column element on another row
void moveToAfterCol_OnAnotherRow(eRow* old, eColumn* x, eRow* new, eColumn* before){
    eColumn* prevX;// declare prevX to save element before x
    if(x != old->col){ // if x its not the first element of the row
        searchPrevCol(&prevX, &x, old); // call procedure searchPrevCol to get the previous element
        prevX->next_col = x->next_col; // prevX point to element after x
    ...
    x->next_col = before->next_col; // x point to element after the before element
    before->next_col = x; // before point to x
}

// procedure to move to last place on another row
void moveToLastCol_OnAnotherRow(eRow* old, eColumn* x, eRow* new){
    eColumn* pointCol = new->col; // declare pointCol, start from col (first)
    while(pointCol->next_col != NULL){ // loop while pointCol didnt point to NULL
        pointCol = pointCol->next_col; // update pointCol with element after pointCol, to do an increment
    }
    // call procedure moveToAfterCol_OnAnotherRow, element before (pointcol) is the last element
    moveToAfterCol_OnAnotherRow(old, x, new, pointCol);
}

// procedure to find column that connect to list of list
void findCol(char tmp1[], char tmp2[], column new, list *L){
    eRow* point = (*L).first; // declare row element point, start from first
    // loop while its not the same with tmp1
    while(point != NULL && strcmp(point->container.product, tmp1) != 0){
        point = point->next; // update point with element after point
    }
    
    eColumn* pointCol = point->col; // declare column element pointCol, start from col (first)
    // loop while its not the same with new.name
    while(pointCol != NULL && strcmp(pointCol->container_col.name, new.name) != 0){
        pointCol = pointCol->next_col; // update pointCol with element after pointCol
    }
    
    eRow* point2 = (*L).first; // declare row element point2, start from first
    // loop while its not the same with tmp2
    while(point2 != NULL && strcmp(point2->container.product, tmp2) != 0){
        point2 = point2->next; // update point2 with element after point2
    }
    // call move procedure
    // moveToLastCol_OnAnotherRow(point, pointCol, point2);

    // update value
    // pointCol->container_col.year = new.year;
}

// procedure to delete an empty row (having no column)
void delEmptyRow(list *L){
    eRow* prev = NULL; // declare prev, start with NULL
    eRow* point = (*L).first; // declare point, start from first
    while(point != NULL){ // loop while point its not NULL
        eRow* nextPoint = point->next; // declare nextPoint to save element after point
        searchPrevRow(&prev, &point, &(*L)); // call procedure searchPrevRow to get element before point

        if(countColElm(*point) == 0){ // if column is empty
            /*call procedure delAfterRow with prev
            (element before point) so point will be deleted*/
            delAfterR(&(*prev));
        }
        point = nextPoint; // update point with element after point
    }
}
